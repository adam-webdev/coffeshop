...
@section('content')
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Order Bahan Baku  </h1>
    ...
    <div class="row">
        <div class="col-md-4 mt-2">
            <div class="card ">
                <h5 class="pt-4 pl-2">Tambah Data</h5>
                <form action="{{ route('orderbahanbaku.store') }}" method="POST">
                    @csrf
                    <div class="modal-body">
                        <div class="form-group">
                            ...
                            <select name="bahanbaku_id" id="bahan_baku" class="form-control select">
                                ...
                                @foreach ($bahanbaku as $bb)
                                    <option value="{{ $bb->id }}" {{ $id !== null && $id == $bb->id ? 'selected' : '' }}>{{ $bb->nama }} </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="petugas" class="form-control" id="petugas" required>
                        </div>
                        <div class="form-group">
                ...
                <div class="d-sm-flex align-items-center justify-content-between mb-4">
                    <div class="table-responsive">
                        <table class="table  table-bordered table-responsive" id="dataTable" width="100%" cellspacing="0">
                            ...
                            <tbody>
                                @foreach ($orderbahanbaku as $obb)
                                    <tr align="center">
                                        <td width="2%">{{ $loop->iteration }}</td>
                                        <td>{{ $obb->bahanbaku->nama }}</td>
                                        <td>{{ $obb->jumlah }}</td>
                                        <td>@currency($obb->harga)</td>
                                        <td>@currency($obb->total_harga)</td>
                                        <td>{{ $obb->petugas }}</td>
                                        <td>{{ \Carbon\Carbon::parse($obb->tanggal)->isoFormat('dddd, D MMMM Y') }}</td>
                                        <td>{{ $obb->supplier }}</td>
                                        <td>{{ $obb->keterangan }}</td>

                                        <td align="center" width="10%">
                                            {{-- @role('Admin') --}}
                                            {{-- <a href="{{ route('orderbahanbaku.edit', [$obb->id]) }}" data-toggle="tooltip"
                                                title="Edit"
                                                class="d-none  d-sm-inline-block btn btn-sm btn-secondary shadow-sm">
                                                <i class="fas fa-edit fa-sm text-white"></i>
                                            </a> --}}
                                            <a href="/orderbahanbaku/hapus/{{ $obb->id }}" data-toggle="tooltip"
